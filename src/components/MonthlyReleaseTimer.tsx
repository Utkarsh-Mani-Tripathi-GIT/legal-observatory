...
function AnimatedDigit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center flex-1">
      <div className="relative w-full overflow-hidden rounded-xl bg-slate-800/80 border border-slate-700/60 py-3 text-center backdrop-blur-sm shadow-inner">
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            className="block font-mono text-3xl font-black text-indigo-400 tabular-nums"
          >
            {display}
          </motion.span>
        </AnimatePresence>
      </div>
      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1.5">
        {label}
      </span>
    </div>
  );
...
export default function MonthlyReleaseTimer({
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden min-h-[420px] flex">
      <AnimatePresence mode="wait">
        {isReleased ? (
          ...
          <motion.div
            ...
            className="flex flex-col justify-between p-8 bg-white dark:bg-slate-900 border-2 border-indigo-500/40 dark:border-indigo-500/30 rounded-2xl shadow-xl hover:shadow-indigo-500/10 transition duration-300 relative group flex-grow w-full"
          >
            <div className="absolute -top-3 left-6 px-3 py-0.5 rounded-full bg-indigo-600 text-white text-[10px] font-bold tracking-widest uppercase flex items-center gap-1 shadow-md animate-bounce-once">
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
              ...
              <Link
                ...
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-lg transition shadow-md flex items-center gap-1.5"
              >
          ...
          <motion.div
            ...
            className="relative flex flex-col justify-between p-8 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl shadow-xl border border-indigo-500/30 overflow-hidden w-full"
          >
            ...
            {/* Center Content */}
            <div className="my-auto py-6 text-center z-10 space-y-4">
              ...
              <AnimatePresence mode="wait">
                {showEarlyAccess ? (
                  <motion.div
                    ...
                    className="bg-indigo-900/50 border border-indigo-500/40 p-4 rounded-xl backdrop-blur-md space-y-2"
                  >
                    ...
                    {subscribed ? (
                      <div className="flex items-center gap-2 text-emerald-400 text-xs font-semibold py-1">
                        <CheckCircle2 className="w-4 h-4" /> You&apos;re on the list! Access link will land in your inbox at 6:00 PM.
                      </div>
                    ) : (
                      <div className="space-y-1.5">
                        <form onSubmit={handleSubscribe} noValidate className="flex gap-2">
                          <input
                            ...
                            className={`bg-slate-950/80 border px-3 py-1.5 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none flex-grow transition-colors ${
                          ...
                          <button
                            type="submit"
                            disabled={loading}
                            className="px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-bold rounded-lg transition shrink-0 shadow-sm"
                          >
                  ...
                  >
                    <p className="text-[11px] text-slate-400 flex items-center justify-center gap-2">
                      <span className="w-5 h-5 flex items-center justify-center rounded bg-slate-900 border border-yellow-500/40 text-[9px] font-serif font-black text-yellow-400 shrink-0 shadow-sm">
                        NLO
                      </span>
